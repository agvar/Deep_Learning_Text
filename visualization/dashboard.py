...
def get_dfs_s3() :
    ...
    try:
        prev_sentiment_counts=pd.read_csv(StringIO(s3_file_sentiment_counts.get()['Body'].read().decode('utf-8')))
        prev_sentiment_counts=prev_sentiment_counts['count'].values.tolist()
...
print(f'version 1 {prev_df_sentiment_day}')
try:
